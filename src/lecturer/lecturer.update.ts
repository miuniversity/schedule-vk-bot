...
export class LecturerUpdate {
  ...
  async searchLecturer(
    ...
    if (ctx.eventPayload.event !== EVENTS.SELECT_LECTURER) {
      return next();
    }
    if (!!ctx.eventPayload.lecturer_id) {
      return next();
    }
  ...
  async getLecturerSchedule(
    ...
    if (ctx.eventPayload.event !== EVENTS.SCHEDULE_LECTURER) {
      return next();
    }

    const lecturer_id = ctx.eventPayload.lecturer_id;
    const date = new Date(
      ctx.eventPayload.date === 'current' ? Date.now() : ctx.eventPayload.date,
    );
    const selected_lecturer = await this.lecturerService.getLecturerSchedule(
      lecturer_id,
      date,
    );
    if (!ctx.eventPayload.noDelete) {
      ...
      await this.vk.api.messages.edit({
        peer_id: ctx.peerId,
        cmid: ctx.conversationMessageId,
        message: this.lecturerService.prepareTextMessageForLecturer(
          selected_lecturer,
          date,
        ),
